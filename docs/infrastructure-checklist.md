...
- Client: DNS interno del Core; DHCP solo con DNS rapido/stabile; IPv6 disabilitato
  se richiesto da TiDental/Dexis; hostname invariato; VPN/mesh/adapter/hosts.
...
| `clientRequiresIpv6Disabled`, `dicomRequired` | `true`/`false` secondo installazione; `null` significa da confermare |
...
- IPv6: indirizzi osservati sono una violazione quando la disabilitazione e'
  richiesta; assenza di indirizzi non dimostra che il binding sia disabilitato.
